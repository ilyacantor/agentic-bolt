...
function CommandCenter(){
  const [m,setM] = React.useState({roi:0,automation:0,uptime:0,efficiency:0});
  React.useEffect(()=>{ fetch('./src/data/metrics.json').then(r=>r.json()).then(setM); },[]);
  return (
    <div className="p-5 w-full">
      <div className="max-w-7xl mx-auto space-y-6">
        <h2 className="text-xl font-semibold">Enterprise Command Center</h2>
        <div className="grid md:grid-cols-4 gap-4">
          <MetricTile label="ROI Improvement" value={`${m.roi}%`} delta="2.1%"/>
          <MetricTile label="Process Automation" value={`${m.automation}%`} delta="1.2%"/>
          <MetricTile label="System Uptime" value={`${m.uptime}%`} delta="Stable"/>
          <MetricTile label="Operational Efficiency" value={`${m.efficiency}%`} delta="0.8%"/>
        </div>
